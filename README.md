...
<title>Bargain Spaza</title>
<style>
/* ====== Reset & base ====== */
* { box-sizing: border-box; margin:0; padding:0; }
body { font-family: "Poppins", Arial, sans-serif; background:#fff8f6; color:#222; }
...
header {
  ...
  align-items:center;
  padding:16px 34px;
  background: linear-gradient(90deg, #0033cc, #6a1b9a, #ff0000, #ff8a00, #ffcc00, #00a651);
  color:white;
  box-shadow:0 4px 10px rgba(0,0,0,0.12);
}
.header-left { display:flex; align-items:center; gap:12px; }
.header-left img.logo-circle {
  width:52px; height:52px; border-radius:50%; object-fit:cover; border:3px solid rgba(255,255,255,0.85);
}
header h1 { font-size:22px; font-weight:700; letter-spacing:0.6px; color:white; }
nav ul { list-style:none; display:flex; gap:18px; align-items:center; }
nav li { cursor:pointer; font-weight:700; color:rgba(255,255,255,0.95); transition:transform .15s; }
nav li:hover { transform:scale(1.02); color:#fffdd0; }

/* ====== Hero ====== */
.hero {
  max-width:1000px; margin:36px auto; background:#fff; border-radius:12px;
  padding:34px; text-align:center; box-shadow:0 6px 18px rgba(0,0,0,0.06);
}
.hero h2 { color:#c8102e; font-size:28px; margin-bottom:10px; }
.hero p { color:#333; line-height:1.5; margin-bottom:16px; }
.hero .cta { background:#ffcc00; color:#000; padding:10px 18px; border:none; border-radius:8px; font-weight:700; cursor:pointer; }

/* ====== Content: slider + article ====== */
.content-section {
  max-width:1200px; margin:28px auto; display:flex; gap:20px; padding:0 20px; align-items:flex-start;
}
/* slider (left 65%) */
.slider {
  flex:0 0 65%;
  position:relative; border-radius:12px; overflow:hidden; box-shadow:0 6px 20px rgba(0,0,0,0.08);
  background:#fff;
  height:400px;
}
...
.slide img { width:150%; height:100%; object-fit:cover; display:block; }
...
<section class="content-section">
  <!-- Slider (left) -->
  <div class="slider" aria-label="Featured products slider">
    <div class="slides">
...
<script>
// Slider logic (auto + manual + clickable slide -> next)
(function(){
  ...
  // click slide to go next
  slides.forEach(s => s.addEventListener('click', next));

  // arrows
  document.getElementById('prevBtn').addEventListener('click', function(e){ e.stopPropagation(); prev(); });
  document.getElementById('nextBtn').addEventListener('click', function(e){ e.stopPropagation(); next(); });

  function resetTimer(){
    if (timer) clearInterval(timer);
    timer = setInterval(next, 4000); // 4s
  }
